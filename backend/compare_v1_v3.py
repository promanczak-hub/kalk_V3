...
async def main():
    ...
    async with async_session_maker() as session:
        result = await session.execute(
            select(LTRKalkulacja).where(LTRKalkulacja.id == calculation_id)
        )
        ...
        params = {"okres_mc": 36, "przebieg_roczny": 50000, "wplyw_na_wr_mc": 0}
        ...
        print(
            f"Cena 1 kpl. opon (tires_capex_net): {res.cena_zakupu.tires_capex_net:.2f}"
        )
        ...
        print(
            f"WR %: {(res.utrata_wartosci.WartoscRezydualnaNaKoniec / res.utrata_wartosci.podstawa_liczenia_v1) * 100 if getattr(res.utrata_wartosci, 'podstawa_liczenia_v1', 0) else 0:.2f}%"
        )
