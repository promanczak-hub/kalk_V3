...
class ExcelOfferGenerator:
    ...
    def generate_offer(
        ...
        for idx, item in enumerate(items):
            ...
            ws_summary.cell(
                row=current_row, column=4, value=item.get("vin_or_config", "")
            )
            ...
            if idx == cheapest_idx:
                recommendation = (
                    ...
                    else f"{recommendation} + Najniższa Rata"
                )
        ...
        for item in items:
            ...
            # Title
            ws["A1"] = (
                f"Specyfikacja Pojazdu: {item.get('brand', '')} {item.get('model', '')}"
            )
            ...
            ws.cell(
                row=current_row, column=1, value="Wyposażenie Standardowe"
            ).font = Font(bold=True)
            current_row += 1
            if not standard_eq:
            ...
            ws.cell(
                row=current_row, column=1, value="Opcje Fabryczne (Płatne)"
            ).font = Font(bold=True)
            current_row += 1
            if not factory_opt:
                ws.cell(row=current_row, column=1, value="Brak danych")
            ...
            ws.cell(
                row=current_row, column=1, value="Opcje Dealerskie / Dodatkowe"
            ).font = Font(bold=True)
            current_row += 1
            if not dealer_opt:
                ws.cell(row=current_row, column=1, value="Brak danych")
