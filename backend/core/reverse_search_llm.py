...
*   `lashing_eyes` (Punkty / uchwyty do mocowania ładunku)
*   `led_cargo_lighting` (Oświetlenie LED przestrzeni ładunkowej)
*   `roof_rack_bars` (Bagażnik dachowy / poprzeczki zintegrowane)
*   `ladder_on_rear_door` (Drabinka na tylnych drzwiach)
*   `step_rear_bumper` (Stopień zintegrowany z tylnym zderzakiem)

**MODYFIKACJE I UŻYTKOWOŚĆ (LCV)**
*   `reinforced_suspension` (Wzmocnione zawieszenie)
*   `engine_sump_guard` (Osłona miski olejowej / dolotowej pod silnikiem)
...
*   `speed_limiter_100_120` (Ogranicznik prędkości 100-120 km/h)
*   `all_weather_tires` (Opony wielosezonowe / całoroczne)
*   `winch` (Wyciągarka)
*   `cargo_rails` (Szyny mocujące w przestrzeni ładunkowej)
*   `wheel_arch_covers` (Obudowy wnęk kół (nadkoli) z drewna lub tworzywa)
...
class ExtractedReverseSearchFeatures(BaseModel):
    ...
    lashing_eyes: Optional[bool] = Field(None)
    led_cargo_lighting: Optional[bool] = Field(None)
    roof_rack_bars: Optional[bool] = Field(None)
    ladder_on_rear_door: Optional[bool] = Field(None)
    step_rear_bumper: Optional[bool] = Field(None)

    reinforced_suspension: Optional[bool] = Field(None)
    engine_sump_guard: Optional[bool] = Field(None)
