...
with open(OUT, "w", encoding="utf-8") as f:
    ...
    f.write(
        f"cenaKatalogowaZOpcjami:         {v1_cena_z_opcjami:>12.2f}  (baza+opcje brutto)\n\n"
    )

    f.write(f"Rabat V1:                       {RABAT_V1 * 100:.0f}%\n")
    v1_po_rabacie = (
        (v1_cena_z_opcjami - OPLATA_TRANSPORTOWA_BRUTTO - OPCJE_NIERABATOWANE_BRUTTO)
        * (1 - RABAT_V1)
        + OPLATA_TRANSPORTOWA_BRUTTO
        + OPCJE_NIERABATOWANE_BRUTTO
    )
    f.write(f"poRabacie (brutto):             {v1_po_rabacie:>12.2f}\n")

    v1_rabat_kwotowo = v1_cena_z_opcjami - v1_po_rabacie
    f.write(
        f"rabatKwotowo:                   {v1_rabat_kwotowo:>12.2f}  (V1 screenshot=55499)\n\n"
    )
    ...
    if CZY_GPS:
        ...
        f.write(
            f"GSM opcja serwisowa (brutto):   {v1_gsm_brutto:>12.2f}  ({GSM_URZADZENIE_NETTO}+{GSM_MONTAZ_NETTO})*{VAT}\n"
        )

    v1_opony_brutto = KOSZT_1KPL_OPON_NETTO * VAT
    f.write(
        f"Opony 1 komplet (brutto):       {v1_opony_brutto:>12.2f}  {KOSZT_1KPL_OPON_NETTO}*{VAT}\n\n"
    )
    ...
    f.write(
        f"Opony capex (netto):            {v3_tires_capex:>12.2f}  ← NIE DODANE!\n\n"
    )
    ...
    f.write(f"V1 BEZ opon (netto):            {v1_bez_opon_netto:>12.2f}\n")
    f.write(
        f"Różnica (V3_netto - V1_netto):  {v3_total_netto - v1_bez_opon_netto:>12.2f}\n"
    )
    ...
    f.write(
        f"Różnica przy tym samym 23%:     {v3_total_23 - v1_bez_opon_netto:>12.2f}\n"
    )
