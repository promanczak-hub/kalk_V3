...
def fix_discount_for_octavia():
    ...
    if not response.data:
        ...
        matches = [
            ...
            if row.get("synthesis_data", {}).get("digital_twin", {}).get("offer_number")
            == offer_number
        ]
        if not matches:
    ...
    print(
        json.dumps(updated_data.get("card_summary", {}), indent=2, ensure_ascii=False)
    )

    print("Saving to DB...")
    update_res = (
        supabase.table("vehicle_synthesis")
        .update({"synthesis_data": updated_data})
        .eq("id", veh_id)
        .execute()
    )
