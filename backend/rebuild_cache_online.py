...
def main():
    ...
    while True:
        try:
            ...
            res = supabase.table("vehicle_matrix_cache").delete().gte("duration_months", 0).execute()
    ...
    print(f"-> Zakończono czyszczenie. Łącznie usunięto: {total_deleted} komórek matrycy.\n")
    ...
    try:
        ...
        print(f"Znaleziono {len(all_ids)} pojazdów przeznaczonych do ponownego przeliczenia z JSONów.")
    except Exception as e:
    ...
    for i in range(0, len(all_ids), batch_size):
        ...
        print(f"-> Podawanie do Workera Celery paczki zadań: {i+1} do {min(i+batch_size, len(all_ids))} ...")
    ...
    print("Sprawdź konsolę Celery (tam gdzie odpalone jest run_dev.py lub worker), by obserwować postępy generowania matryc!")
