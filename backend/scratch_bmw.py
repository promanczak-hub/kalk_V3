...
res = client.table('vehicle_synthesis').select('id, brand, model, verification_status, semantic_embedding, synthesis_data').execute()
...
for r in res.data:
    ...
    if "ŚREDNIA" in samar_cat or "SREDNIA" in samar_cat or "SUV" in samar_cat:
        if "Terenowo-rekreacyjne" in samar_cat and "D" in samar_cat:
            ...
            targets.append((r['brand'], r['model'], samar_cat, has_emb, r['verification_status']))
