...
async def main():
    ...
    print(
        f"synth -> card_summary: {json.dumps(card_sum, ensure_ascii=False, indent=2)}"
    )

    base_input = build_calculator_input(
        vehicle_row, 0.15, settings
    )  # forced 15% discount for testing, but let's see what is inside

    print(
        f"Base Input built. Pricing_margin_pct: {base_input.financial_params.pricing_margin_pct}"
    )
    ...
    print(
        f"Base Input built. sales_prep: {base_input.financial_params.sales_prep_correction}"
    )
