...
from core.matrix_cache_job import build_calculator_input, process_single_kalkulacja_matrix_task
...
def run_refresh_and_report() -> None:
    ...
    res = supabase.table("vehicle_synthesis").select(
        "id, brand, model, verification_status, created_at, synthesis_data"
    ).execute()
    ...
    for i, row in enumerate(vehicles):
        ...
        try:
            ...
            if exist_res.data:
                ...
                supabase.table("ltr_kalkulacje").update(kalk_data).eq("id", kalk_id).execute()
            else:
        ...
        try:
            ...
            if captured_error:
                ...
                successes.append({"vehicle_id": vid, "label": label, "created_at": created_at})
