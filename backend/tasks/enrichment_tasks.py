...
from core.cross_ref_evidence import create_evidence_batch, create_body_param_evidence, save_catalog_match
...
def enrich_vehicle_features_from_catalog(vehicle_id: str) -> dict:
    ...
    # 1. Fetch vehicle
    v_resp = sb.table("vehicle_synthesis").select("id, synthesis_data").eq("id", vehicle_id).limit(1).execute()
    if not v_resp.data:
    ...
    docs_resp = sb.schema("reverse_search").table("model_document_sources")\
        .select("id")\
    ...
    if not exact_variant:
        logger.info(f"No 100% exact match found for {vehicle_id}. Falling back to standard LLM enrichment from PDF spec.")
    ...
    logger.info(f"Background enrichment found EXACT match: {exact_variant.get('variant_name')} for {vehicle_id}")
    ...
    logger.info(f"Strict background enrichment complete for {vehicle_id}. Extracted {len(match_result.features)} features.")
