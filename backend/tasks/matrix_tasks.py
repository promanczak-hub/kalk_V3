...
def process_kalkulacja_matrix_task(
    self, kalkulacja_id: str, trace_id: str | None = None
) -> str:
    """
    ...
    logger.info(
        "Executing Celery task for kalkulacja_id: %s [Trace: %s]",
        kalkulacja_id,
        task_trace_id,
    )
    try:
        process_single_kalkulacja_matrix_task(
            kalkulacja_id, celery_task_id=self.request.id, trace_id=task_trace_id
        )
        return f"Cache refreshed for kalkulacja {kalkulacja_id}"
    except Exception as e:
        logger.error(
            "Failed to process kalkulacja matrix in Celery task: %s [Trace: %s]",
            e,
            task_trace_id,
        )
        raise
