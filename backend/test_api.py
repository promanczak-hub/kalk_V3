...
if res.data:
    for v in res.data:
        ...
        try:
            r1 = requests.get(f'http://localhost:8000/api/kalkulator/pojazd/{vid}', timeout=5)
            print('  /api/kalkulator STATUS:', r1.status_code)
        except Exception as e:
            print('  /api/kalkulator ERROR:', e)

        try:
            r2 = requests.get(f'http://localhost:8000/api/features/vehicle/{vid}/state', timeout=15)
