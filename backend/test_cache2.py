...
async def main():
    res = supabase.table('vehicle_synthesis').select('id, brand, model, samar_id').eq('samar_id', 'C9DQAAPO').execute()
    vehicles = res.data
    for v in vehicles:
        ...
        all_cache = supabase.table('vehicle_matrix_cache').select('margin_pct, duration_months, annual_mileage, monthly_price_net').eq('vehicle_id', v['id']).execute()
