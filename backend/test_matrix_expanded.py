...
if __name__ == "__main__":
    ...
    res_cache = supabase.table("vehicle_matrix_cache").select("vehicle_id").limit(1).execute()
    if res_cache.data:
        vid = res_cache.data[0]["vehicle_id"]
        print(f"Testing full refresh for vehicle: {vid}")
        
        # Przed
        before = supabase.table("vehicle_matrix_cache").select("annual_mileage").eq("vehicle_id", vid).execute()
        ...
        # Po
        after = supabase.table("vehicle_matrix_cache").select("annual_mileage").eq("vehicle_id", vid).execute()
