...
try:
    ...
    res = sb.schema("reverse_search").table("vehicle_feature_state").select("feature_id, resolved_value_num, reverse_search:universal_features(feature_key)").eq("source_vehicle_id", vehicle_id).execute()
    ...
    for r in res.data:
        ...
        if val is not None:
            feature_key = r.get("reverse_search", {}).get("feature_key") if r.get("reverse_search") else None
            ...
            if not feature_key:
                ...
                relevant = ["length_mm", "width_mm", "height_mm", "wheelbase_mm", "dmc_kg", "curb_weight_kg", "payload_kg"]
