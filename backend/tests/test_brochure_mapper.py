...
def test_build_brochure_from_v1():
    data = {
        ...
        "optional_equipment": [{"name": "Lakier metalik", "price": 4000.0}],
        "packages": [
            {
    ...
    std_cat = next(
        cat for cat in brochure.equipment_categories if cat.category_name == "Wnętrze"
    )
    ...
    opt_cat = next(
        cat
        for cat in brochure.equipment_categories
        if cat.category_name == "Opcje dodatkowe"
    )
    ...
    pkg_cat = next(
        cat
        for cat in brochure.equipment_categories
        if cat.category_name.startswith("Pakiet: Pakiet Comfort")
    )
...
def test_build_brochure_from_v2():
        ...
        "paid_options": [
            {
                ...
                "price": "1000 PLN",
                "price_type": "netto",
                "category": "Opcjonalne V2",
            },
            {
                "name": "Podłoga ze sklejki",
                "price": "1500 PLN",
                "price_type": "netto",
                "category": "Zabudowa",
            },
    ...
    std_cat = next(
        cat
        for cat in brochure.equipment_categories
        if cat.category_name == "Wyposażenie standardowe"
    )
    ...
    opt_cat = next(
        cat
        for cat in brochure.equipment_categories
        if cat.category_name == "Opcjonalne V2"
    )
    assert "Kamera cofania (1000 PLN)" in opt_cat.items

    zab_cat = next(
        cat for cat in brochure.equipment_categories if cat.category_name == "Zabudowa"
    )
    ...
    util_cat = next(
        cat
        for cat in brochure.equipment_categories
        if cat.category_name == "Cechy użytkowe (Wymiary i Masy)"
    )
