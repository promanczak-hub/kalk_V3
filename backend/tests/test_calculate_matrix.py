...
def test_calculate_matrix_sanity():
    ...
    with patch('core.LTRKalkulator.get_vehicle_from_db', return_value={
        "id": "mock-vehicle",
        "samar_class_id": 1,
        "engine_type_id": 1,
        "power_kw": 110.0,
        ...
        "model": "Corolla"
         ...
         patch('core.LTRKalkulator.get_insurance_rates_from_db', return_value=[{"KolejnyRok": i, "SkladkaOC": 100.0, "StawkaBazowaAC": 0.05, "StawkaNW": 50.0} for i in range(1, 8)]), \
         ...
         patch('core.LTRSubCalculatorSerwisNew.get_service_rate_from_db', return_value={
             "cost_aso_per_km": 0.15,
             "cost_non_aso_per_km": 0.10
         }), \
         patch('core.samar_rv.SamarRVCalculator._fetch_depreciation_rates', return_value={0: {"base": 0.5, "options": 0.5}, 4: {"base": 0.0, "options": 0.0}}), \
         ...
         patch('core.samar_rv.SamarRVCalculator._fetch_class_config', return_value={"name": "mock"}), \
         patch('core.samar_rv.SamarRVCalculator.fetch_color_correction', return_value=0.0), \
         patch('core.samar_rv.SamarRVCalculator.fetch_body_correction', return_value=(0.0, 0.0)), \
        ...
        mock_tires.return_value.calculate_cost.return_value = {
            "capex_initial_set": 0, "monthly_hardware": 0, "monthly_storage": 0, "monthly_swaps": 0, "IloscOpon": 4
        }
    ...
    assert found_custom, "Siatka nie zawiera wejściowych parametrów użytkownika (48mc / 20k km)."
