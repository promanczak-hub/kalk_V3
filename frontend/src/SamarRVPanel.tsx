...
const SamarClassRow = ({ row, onSave }: { row: SamarClass, onSave: (r: SamarClass) => void }) => {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {editing ? (
          <input type="number" step="0.01" value={stawka} onChange={e => setStawka(parseFloat(e.target.value))} style={{ width: 80 }} />
        ) : (
      ...
      <TableCell>
        {editing ? (
          <input type="number" step="10" value={koszt} onChange={e => setKoszt(parseFloat(e.target.value))} style={{ width: 100 }} />
        ) : (
      ...
      <TableCell align="right">
        {editing ? (
          <button onClick={() => { onSave({ ...row, stawka_serwis_km: stawka, koszt_przegladu_podstawowego: koszt }); setEditing(false); }}>Zapisz</button>
        ) : (
          <button onClick={() => setEditing(true)}>Edytuj Konfig. Serwisu</button>
        )}
      </TableCell>
    </TableRow>
  )
...
export default function SamarRVPanel() {
  ...
  const fetchData = async () => {
    try {
      const [cls, bas, brd, age, mil] = await Promise.all([
        axios.get<SamarClass[]>('http://127.0.0.1:8000/api/samar-rv/classes'),
        axios.get<BasePercentage[]>('http://127.0.0.1:8000/api/samar-rv/base-percentages'),
        axios.get<BrandCorrection[]>('http://127.0.0.1:8000/api/samar-rv/brand-corrections'),
        axios.get<AgeDepreciation[]>('http://127.0.0.1:8000/api/samar-rv/depreciation'),
        axios.get<MileageCorrection[]>('http://127.0.0.1:8000/api/samar-rv/mileage')
      ]);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Nieznany błąd podczas pobierania danych SAMAR RV.';
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        Poniższe tabele pochodzą docelowo z V1 MSSQL. Kalkulator odpytuje o nie przez API. Aby zmienić wartości, należy użyć narzędzi bazodanowych z uwagi na wymaganą historyzację LTR.
      </Alert>
      ...
      <Grid container spacing={2}>
          ...
          <Accordion>
            ...
            <AccordionDetails>
              <Table size="small">
                ...
                <TableBody>
                  {classes.map(c => (
                    <SamarClassRow key={c.id} row={c} onSave={async (updatedClass) => {
                      try {
                        const res = await axios.post('http://127.0.0.1:8000/api/samar-rv/classes', updatedClass);
                        setClasses(prev => prev.map(p => p.id === res.data.id ? res.data : p));
        ...
        <Grid size={{ xs: 12 }}>
          <Accordion>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography fontWeight="bold">Tabela 2: Wartości Bazowe (% RV)</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Table size="small">
                <TableHead>
                ...
                <TableBody>
                  {basePct.map(b => (
                    <TableRow key={b.id}>
                      ...
                      <TableCell>{(b.korekta_procent * 100).toFixed(2)} %</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <Grid size={{ xs: 12 }}>
          <Accordion>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography fontWeight="bold">Tabela 3: Korekta za Wiek (Lata)</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Table size="small">
                <TableHead>
                ...
                <TableBody>
                  {ageDepr.map(a => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell>{(a.korekta_procent * 100).toFixed(2)} %</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <Grid size={{ xs: 12 }}>
          <Accordion>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography fontWeight="bold">Tabela 4: Korekta za Przebieg</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Table size="small">
                <TableHead>
                ...
                <TableBody>
                  {mileageCorr.map(m => (
                    <TableRow key={m.id}>
                      ...
                      <TableCell>{(m.korekta_procent_ponizej_190 * 100).toFixed(4)} %</TableCell>
                      <TableCell>{(m.korekta_procent_powyzej_190 * 100).toFixed(4)} %</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <Grid size={{ xs: 12 }}>
          <Accordion>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography fontWeight="bold">Tabela 5: Korekta (Narzut) na Markę</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Table size="small">
                <TableHead>
