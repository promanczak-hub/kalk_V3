    ...
    <div className="w-full flex flex-col gap-2 relative">
      {isCreating && (
        <div className="w-full h-1 bg-slate-200 rounded-none overflow-hidden">
           <div
             className="h-full bg-slate-900 rounded-none"
             style={{ width: `${progress}%`, transition: phase === 'done' ? 'width 0.3s ease-out' : 'width 1.5s cubic-bezier(0.4, 0, 0.2, 1)' }}
           />
        </div>
      )}
      {isCreateBlocked && (
        <div className="text-xs text-red-700 bg-white border border-red-600 rounded-none px-3 py-2">
          {calculationBlockReason}
        </div>
      )}
      ...
      <button
        ...
        className={cn(
          "flex items-center text-xs font-semibold px-4 py-2 rounded-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed",
          isCreating
      ...
      {activeKalkulacjaNumer && !isCreating && (
         <span className="absolute -top-3 right-0 bg-emerald-600 text-white text-[10px] px-2 py-0.5 rounded-none font-bold border border-white z-10">
      ...
      <button
        ...
        className="flex items-center text-xs font-semibold px-4 py-2 rounded-sm bg-white border border-slate-900 text-indigo-700 hover:bg-indigo-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isGeneratingBrochure ? (
      ...
      <button
        ...
        className="flex items-center text-xs font-semibold px-4 py-2 rounded-sm bg-white border border-slate-900 text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition-colors"
      >
      ...
      {vehicle.raw_pdf_url && (
        <>
          <button
            ...
            className={cn(
              "flex items-center text-xs font-semibold px-4 py-2 rounded-sm transition-colors border",
              isViewerOpen
                ? "bg-slate-200 border-slate-900 text-slate-700 hover:bg-slate-300"
                : "bg-white border-slate-900 text-blue-700 hover:bg-blue-50"
            )}
          >
      ...
      <button
        ...
        className="flex items-center text-xs font-semibold px-4 py-2 rounded-sm bg-white border border-slate-900 text-red-600 hover:bg-red-50 hover:border-red-600 transition-colors"
      >
