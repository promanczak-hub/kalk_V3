...
opt = (
    ...
    .select("samar_class_id, engine_type_id")
    .execute()
    .data
)
...
for c in classes:
    ...
    if class_id not in ub_set:
        errors.append(
            f"[Ubezp] Brak stawek ubezpieczenia dla klasy: {c['name']} (ID {c['id']})"
        )

    if class_id not in dep_class_set:
        errors.append(
            f"[Deprec] Brak wiersza amortyzacji dla: {c['name']} (ID {c['id']})"
        )
    ...
    for e in engines:
        ...
        if (class_id, engine_id) not in opt_set:
            errors.append(
                f"[Opcje] Brak mnoznika opcji RV dla: {c['name']} (ID {c['id']}), silnik: {e['name']} (ID {e['id']})"
            )
