...
from api.extract_routes import get_vehicle_synthesis

print('Fetching all vehicles...')
vehicles = supabase.table('vehicle_synthesis').select("id, brand, model, synthesis_data").execute().data
...
for v in vehicles:
    ...
    if not samar_class_name or not engine_name:
        fails += 1
        print(f"[{v['id']}] {brand_name} {v['model']} - BRAK samar_class_name LUB engine_name w mapped_ai_data!")
        continue
    ...
    try:
        ...
        if not res.get("is_ready"):
            fails += 1
            print(f"[{v['id']}] {brand_name} {v['model']} - NOT READY: {res.get('reasons')}")
