...
vehicles = supabase.table('vehicle_synthesis').select('*').in_('offer_number', ['CYJT98P6', 'CSVZ5HMV']).execute().data
for v in vehicles:
    ...
    cell2 = next((c for c in cells2 if c['Okres'] == 48 and c['Przebieg'] == 30000), None)
    if cell2:
        print(f"LacznaStawka (with 20% margin inside calculator): {cell2['LacznaStawka']}")
