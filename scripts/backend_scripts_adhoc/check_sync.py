...
async def run():
    ...
    syn = supabase.table('vehicle_synthesis').select('id, brand, model, samar_class_id, created_at').execute()
    ...
    if len(uncalculated) > 0:
        ...
        if len(has_samar) > 0:
            ...
            print(has_samar[['id', 'brand', 'model', 'samar_class_id']].head(15).to_string())
