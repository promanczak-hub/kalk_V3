...
def test_reprocess():
    with open("check_output.txt", "w", encoding="utf-8") as f:
        try:
            ...
            resp = supabase.table("vehicle_synthesis").select("synthesis_data").eq("id", synth_id).execute()
            data = resp.data[0]["synthesis_data"]
            
            f.write("Original utility_features: " + json.dumps(data.get("card_summary", {}).get("utility_features")) + "\n")
            ...
            f.write("New utility_features: " + json.dumps(cs.get("utility_features")) + "\n")
