...
def main():
    ...
    print(
        f"   Pomyślnie wygenerowano wektor - wymiar: {len(vector)} (fragment: {vector[:3]}...)\n"
    )

    vector_str = f"[{','.join(str(v) for v in vector)}]"

    print(
        "3. Przeszukiwanie bazy danych (Hybrid Search: semantyka wg wektora osadzeń)..."
    )
    import httpx
    ...
    for i, row in enumerate(data[:5]):
        ...
        print(
            f"       Opłata max: ~{row.get('best_monthly_price')} PLN (ID: {row.get('vehicle_id')})"
        )
        print()
