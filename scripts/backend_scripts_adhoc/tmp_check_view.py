...
def main():
    vehicle_id = "0b0800ed-ccbb-46f9-bd4d-ac67ff2d3921"  # A random vehicle from previous tests
    
    # Let's just fetch 1 row from the view to see the keys
    resp = sb_client.schema("reverse_search").table("vehicle_features_summary_view").select("*").limit(1).execute()
