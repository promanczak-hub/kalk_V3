...
def test_manual_enrichment():
    ...
    item = {
        ...
        "calculation_data": {
            "kalkulacja_id": calc_id,
            "vehicle_id": vehicle_id
        }
    }
    
    # Manual enrichment logic (from route)
    matrix_res = supabase.table("vehicle_matrix_cache").select("*").eq("kalkulacja_id", calc_id).execute()
    if matrix_res.data:
    ...
    if synth_res.data:
        ...
        item["factory_options"] = [opt for opt in synth.get("digital_twin", []) if "Pakiet" in opt or "lakier" in opt.lower()]
        print(f"Found {len(item['standard_equipment'])} standard options and {len(item['factory_options'])} derived factory options.")

    # Generation
    generator = ExcelOfferGenerator(template_path="core/templates/template_oferta_v2.xlsx")
    excel_bytes = generator.generate_offer(
        client_name="Testowy Klient Sp. z o.o.",
