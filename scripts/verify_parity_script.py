...
async def verify():
    ...
    for v in res.data:
        ...
        cache_check = supabase.table("vehicle_matrix_cache").select("id").eq("vehicle_id", vid).limit(1).execute()
        if cache_check.data:
            best_v = v
            break
    ...
    cache_res = supabase.table("vehicle_matrix_cache").select("*").eq("vehicle_id", vid).eq("margin_pct", 0.0).limit(1).execute()
    ...
    print(f"Cache Entry: {months} mos, {mileage} km/yr | LTR Price: {cache_price} PLN | Stored Base Price: {cache_base_price} PLN")
