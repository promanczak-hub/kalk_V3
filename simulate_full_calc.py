...
class SafeLTRKalkulator(LTRKalkulator):
    def build_matrix(self, only_exact=False):
        ...
        wr_total_net = (base_price_net * wr_base_multiplier) + (total_options_net * wr_options_multiplier)
        ...
        print(f"CAPEX Scenario B (Discount Base Only): {capex_scenario_b:,.2f}") # ~181k
        print("-" * 30)
        print(f"Computed WR (net) [40% WR base / 21% WR options]: {wr_total_net:,.2f}")
        print(f"Target WR (net): 84,477.00")
...
def simulate_skoda_calc():
    factory_options = [
        VehicleOptions(name="Options Total", price_net=28211.38, price_gross=34700.0, include_in_wr=True)
    ]
