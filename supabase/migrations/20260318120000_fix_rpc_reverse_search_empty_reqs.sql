-- Migration: Fix rpc_reverse_search returning 0 results when p_requirements is empty

DROP FUNCTION IF EXISTS reverse_search.rpc_reverse_search(jsonb,text[],text[],integer[],text[]);

CREATE OR REPLACE FUNCTION reverse_search.rpc_reverse_search(
    p_requirements jsonb,
    p_brands text[] DEFAULT NULL,
    p_models text[] DEFAULT NULL,
    p_samar_class_ids integer[] DEFAULT NULL,
...
-- Update public wrapper to match 24 columns
DROP FUNCTION IF EXISTS public.rpc_reverse_search(jsonb,text[],text[],integer[],text[]);
...
LANGUAGE sql
AS $function$
    select * from reverse_search.rpc_reverse_search(p_requirements, p_brands, p_models, p_samar_class_ids, p_trims);
$function$;
