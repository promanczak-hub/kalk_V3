-- Migration: 20260318185000_revert_available_filters_to_scoring.sql
-- Description: Revert rpc_get_available_filters to its previous state (from 20260318110500) because the strict filtering disabled other scoring options (NICE_TO_HAVE).
...
begin
    with filtered_vehicles as (
        ...
        where 
            ...
            and (p_body_types is null or array_length(p_body_types, 1) is null or lower(vs.synthesis_data->'mapped_ai_data'->>'body_type') = any(select lower(unnest) from unnest(p_body_types)))
