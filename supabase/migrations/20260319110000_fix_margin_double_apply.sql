...
RETURNS TABLE(
    vehicle_id uuid, brand text, model text, version text, match_score_pct numeric, 
    matched_features jsonb, missing_features jsonb, best_monthly_price numeric, 
    fuel_type text, power_hp integer, transmission text, body_style text, 
    drive_type text, base_price_gross text, options_price_gross text, 
    total_price_gross text, suggested_discount_pct numeric, trim_level text, 
    vehicle_class text, has_ltr_cache boolean, service_cost_type text, 
    tire_class text, offer_number text, configuration_code text
)
LANGUAGE plpgsql
AS $function$
declare
...
begin
    ...
    vehicle_matrix_best as (
        -- Return raw net price (0% margin) to avoid double-charging margin
        select vmc.vehicle_id as v_id,
               round(min(vmc.monthly_price_net), 2) as best_monthly_price,
               true as has_cache
        ...
        select tv.v_id, tv.brand, tv.model, tv.version,
               ...
               case 
                   ...
                   when req->>'feature_key' = 'monthly_price_net' then
                       case 
                           when req->>'operator' = 'lte' and (vmb.best_monthly_price / (1.0 - coalesce((select req_margin_pct from matrix_context limit 1), 0) / 100.0)) <= (req->>'value')::numeric * 1.05 then true
                           when req->>'operator' = 'gte' and (vmb.best_monthly_price / (1.0 - coalesce((select req_margin_pct from matrix_context limit 1), 0) / 100.0)) >= (req->>'value')::numeric * 0.95 then true
                           else false
                       end
