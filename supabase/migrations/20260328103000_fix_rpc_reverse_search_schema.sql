-- ============================================================
-- Migration: Fix rpc_reverse_search signature in public schema
-- Date: 2026-03-28
--
-- Problem: PostgREST cache sees old signature (p_period, p_price_max,
--          p_price_min, p_vehicle_ids) because old overloads in public
--          schema were never dropped. The new function lives in
--          reverse_search schema but the backend calls public.
--
-- Solution:
--   1. Drop ALL overloads of public.rpc_reverse_search
--   2. Create a single clean version in public with the correct
--      signature matching scoring_search_routes.py
--   3. NOTIFY pgrst reload
-- ============================================================

-- Step 1: Drop all existing overloads of rpc_reverse_search in public schema
DROP FUNCTION IF EXISTS public.rpc_reverse_search(
    jsonb, text[], text[], integer[], text[], uuid[], text, vector
);
DROP FUNCTION IF EXISTS public.rpc_reverse_search(
    jsonb, text[], text[], integer[], text[], uuid[]
);
DROP FUNCTION IF EXISTS public.rpc_reverse_search(
    integer, numeric, numeric, uuid[]
);
DROP FUNCTION IF EXISTS public.rpc_reverse_search(
    uuid[], numeric, numeric, integer
);
DROP FUNCTION IF EXISTS public.rpc_reverse_search(
    p_period integer, p_price_max numeric, p_price_min numeric, p_vehicle_ids uuid[]
);
-- Cascade just in case any other variant exists
DROP FUNCTION IF EXISTS public.rpc_reverse_search CASCADE;

-- Step 2: Create the canonical public.rpc_reverse_search
-- Signature matches scoring_search_routes.py exactly:
--   p_requirements, p_brands, p_models, p_samar_class_ids, p_trims, p_vehicle_ids
-- (search_query and semantic_vector are optional extras — backend doesn't send them yet)
CREATE OR REPLACE FUNCTION public.rpc_reverse_search(
    ...
    p_semantic_query_vector vector(768)   DEFAULT NULL::vector
)
RETURNS TABLE(
    vehicle_id              uuid,
    brand                   text,
    ...
    body_style              text,
    drive_type              text,
    base_price_gross        text,
    options_price_gross     text,
    total_price_gross       text,
...
declare
    ...
    v_search_query_formatted text;
begin
    if p_search_query is not null and trim(p_search_query) != '' then
        v_search_query_formatted := trim(p_search_query);
    end if;
    ...
    with target_vehicles as (
        select vs.id as v_id,
               coalesce(vs.brand, vs.synthesis_data->>'brand') as brand,
               coalesce(vs.model, vs.synthesis_data->>'model') as model,
               vs.semantic_embedding,
        ...
        where 
            ...
            and (p_vehicle_ids is null or array_length(p_vehicle_ids, 1) is null or vs.id = any(p_vehicle_ids))
    ),
    matrix_context as (
        select 
    ...
    vehicle_evals as (
        select tv.v_id, tv.brand, tv.model, tv.version,
               tv.offer_number, tv.configuration_code, tv.semantic_embedding,
    ...
    survivors as (
        select ve.v_id, ve.brand, ve.model, ve.version,
               ve.offer_number, ve.configuration_code, ve.semantic_embedding,
    ...
    select s.v_id as vehicle_id, s.brand, s.model, s.version,
           round(
             (
                 (s.score / v_total_possible_score) * 100 
                 + case 
                     when p_semantic_query_vector is not null and s.semantic_embedding is not null then
                        (1 - (s.semantic_embedding <=> p_semantic_query_vector)) * 30.0
                     else 0 
                   end
                 + case
                     when v_search_query_formatted is not null then
                        coalesce(similarity(s.brand || ' ' || s.model || ' ' || coalesce(s.cs_trim_level, '') || ' ' || coalesce(s.cs_body_style, ''), v_search_query_formatted) * 20.0, 0)
                     else 0
                   end
             )::numeric, 
           2) as match_score_pct,
...
GRANT EXECUTE ON FUNCTION public.rpc_reverse_search(
    jsonb, text[], text[], integer[], text[], uuid[], text, vector
