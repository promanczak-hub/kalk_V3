-- Przywraca logikę punktowania w rpc_reverse_search na 100% gdy brak celowych kryteriów.
...
declare
    ...
    v_has_scorable_features boolean := false;
begin
    if p_search_query is not null and trim(p_search_query) != '' then
        v_search_query_formatted := trim(p_search_query);
    end if;

    select coalesce(sum((r->>'weight')::numeric), 0), count(r->>'feature_key') > 0
    into v_total_possible_score, v_has_scorable_features
    ...
    select s.v_id as vehicle_id, s.brand, s.model, s.version,
           round(
             (
                 case 
                     when not v_has_scorable_features then 100.0
                     else (s.score / v_total_possible_score) * 100 
                 end
                 + case 
