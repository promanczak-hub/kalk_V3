DROP FUNCTION IF EXISTS reverse_search.rpc_reverse_search(jsonb, text[], text[], integer[], text[], uuid[], text, vector);
...
CREATE OR REPLACE FUNCTION reverse_search.rpc_reverse_search(
    ...
    p_search_keywords text[] DEFAULT NULL::text[]
)
RETURNS TABLE(
    vehicle_id uuid,
    brand text,
    ...
    select s.v_id as vehicle_id, s.brand, s.model, s.version,
           round(
             least(100.0, (
                 ...
                 + case
                     when p_search_keywords is not null and array_length(p_search_keywords, 1) > 0 then
                        (select count(*)::numeric 
                         from unnest(p_search_keywords) kw 
                         where s.cs_paid_options::text ilike '%' || kw || '%' 
                            or s.cs_std_equipment::text ilike '%' || kw || '%'
                        ) * 25.0
                     else 0
                   end
             )::numeric), 
