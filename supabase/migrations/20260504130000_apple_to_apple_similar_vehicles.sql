-- Apple-to-apple similarity pricing.
-- For each source vehicle, take the (tire_class, service_type) setup that the
-- main card displays (cheapest + newest, same logic as rpc_get_batch_prices),
-- then resolve each candidate's price using the IDENTICAL setup. If a candidate
-- has no row with the same (duration, mileage, tire_class, service_type),
-- best_monthly_price comes back NULL — frontend treats that as "no comparable
-- offer" instead of silently showing the cheapest unrelated variant.
...
BEGIN
  ...
  WITH source_setup AS (
    -- Mirror rpc_get_batch_prices: pick cheapest + newest variant for the
    -- requested (duration, mileage). That's the "apple" the main card shows.
  ...
  candidate_match AS (
    -- Resolve each candidate's price for the EXACT same (dur, mil, tire, service) setup
