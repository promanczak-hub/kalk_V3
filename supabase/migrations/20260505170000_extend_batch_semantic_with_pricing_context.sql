-- Extend batch semantic similarity RPC with pricing/discount/matrix context.
-- Adds 5 fields to similarity_reasons so the frontend can render an
-- expandable "details" block per candidate without an extra round-trip:
--
--   base_price                 numeric  candidate catalog price (net, post-norm)
--   discount_pct               numeric  candidate offer discount %
--   discount_pct_diff          numeric  candidate.discount - source.discount (pp)
--   matched_duration_months    integer  contract duration the rate was priced for
--   matched_annual_mileage     integer  annual mileage the rate was priced for
--
-- Builds on 20260505150000 (class+body gates). All other behaviour identical.
...
BEGIN
  ...
  source_embeddings AS (
    SELECT
        ...
        -- ── NEW: source's discount % (for diff calculation) ──
        COALESCE(
  ...
  SELECT
    jsonb_build_object(
      ...
      'similarity_reasons', jsonb_build_object(
          ...
          -- ── NEW: pricing context for expandable details block ──
