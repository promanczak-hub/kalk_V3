-- A4b: fuel_match porównuje teraz family (ice/mhev/hev/phev/bev/fcev/lpg) zamiast
-- exact string equality. 1.5 TSI vs 2.0 TSI = match (oba ice). TSI vs mHEV =
-- mismatch (różne family per SOT _dict_powertrains). Frontend label rename osobno
-- (SimilarVehiclesPanel.tsx: "Inny rodzaj napędu" → "Inne paliwo / silnik").
-- Builds on 20260505180000 (passthrough options) + 20260506130000 (fn_powertrain_family).
  ...
  SELECT
    jsonb_build_object(
      ...
      'similarity_reasons', jsonb_build_object(
          ...
          'fuel_match',      (public.fn_powertrain_family(c.cand_fuel) = public.fn_powertrain_family(c.ref_fuel)
                              AND public.fn_powertrain_family(c.ref_fuel) IS NOT NULL),
